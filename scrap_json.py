...
def save_to_json(restaurants):
    data = [{'BusinessID': restaurant['id'], 'CuisineType': restaurant['cuisine']} for restaurant in restaurants]
...
if __name__ == '__main__':
    ...
    for cuisine in cuisine_types:
        ...
        all_restaurants.extend(total_restaurants)
        total_stored_restaurants += len(total_restaurants)

        if total_stored_restaurants >= max_total_restaurants:
            break

    save_to_json(all_restaurants[:100])
    print(f'Saved {len(all_restaurants[:100])} restaurants to restaurants.json')
